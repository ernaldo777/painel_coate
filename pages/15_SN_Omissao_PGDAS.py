...
def _resolver_arquivo_dados() -> Path | None:
    candidatos = [
        _DATA_DIR / "Omissão_PGDAS.xlsx",
        _DATA_DIR / "Omissao_PGDAS.xlsx",
        _DATA_DIR / "Omissão PGDAS.xlsx",
    ]
    ...
    for caminho in sorted(_DATA_DIR.glob("*.xlsx")):
        ...
        if "omiss" in nome and "pgdas" in nome:
            return caminho
    return None
...
st.markdown(
    ...
    + """
        <h1>🚫 Omissão PGDAS</h1>
        <p>
            Monitoramento de contribuintes e competências com omissão de declaração no PGDAS,
...
tot_operacoes = int(df_total["qt_operacoes"].iloc[0]) if not df_total.empty else int(df_anuais["qt_operacoes"].sum())
...
periodo_top_op = (
    df_anuais.loc[df_anuais["qt_operacoes"].idxmax()]
    if not df_anuais.empty
    else None
)
...
with k2:
    st.markdown(
        ...
        <div class="coate-kpi-card accent-warning">
            <div class="coate-kpi-top">
                <div class="coate-kpi-label">Competências Omissas (Total)</div>
                <div class="coate-kpi-icon">📅</div>
            </div>
            <div class="coate-kpi-value">"""
        + _fmt_int(tot_operacoes)
        + """</div>
            <div class="coate-kpi-delta delta-warning">Declarações não entregues</div>
            <div class="coate-kpi-help">Cada competência (mês/ano) sem declaração PGDAS conta individualmente por CNPJ.</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
with k3:
    st.markdown(
        ...
        + """</div>
            ...
            <div class="coate-kpi-help">Quantidade de anos-calendário com dados de omissão.</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
# ── Leitura rápida ─────────────────────────────────────────────────────────────
if periodo_top_cnpj is not None and periodo_top_op is not None:
    st.markdown(
        """
        ...
        + """ do total acumulado).
                O maior volume de <strong>competências omissas</strong> também ocorreu em
                <strong>"""
        + str(periodo_top_op["periodo"])
        + """</strong>, com <strong>"""
        + _fmt_int(periodo_top_op["qt_operacoes"])
        + """</strong> declarações não entregues ("""
        + _fmt_pct(periodo_top_op["perc_operacoes"])
        + """ do total).
            </p>
        </div>
        """,
...
st.markdown(
    ...
    <div class="coate-section">
        ...
        <div class="coate-section-title">CNPJs e Competências Omissas por Ano</div>
    </div>
    <hr class="coate-section-divider"/>
    """,
    unsafe_allow_html=True,
    ...
    for _, row in df_anuais.iterrows():
        fig_op.add_trace(
            go.Bar(
                ...
                name=str(row["periodo"]),
                marker_color=CORES_PERIODO.get(str(row["periodo"]), "#94a3b8"),
                text=[_fmt_int(row["qt_operacoes"])],
                textposition="outside",
                customdata=[[_fmt_pct(row["perc_operacoes"])]],
                hovertemplate="Ano: %{x}<br>Competências: %{text}<br>% do total: %{customdata[0]}<extra></extra>",
            )
        )
    fig_op.update_layout(**_layout_plot(_layout_base, title="Competências Omissas por Ano"))
...
with g4:
    fig_line_op = go.Figure()
    fig_line_op.add_trace(
        go.Scatter(
            x=df_anuais["periodo"].tolist(),
            y=df_anuais["qt_operacoes"].tolist(),
            mode="lines+markers+text",
            marker=dict(color="#f59e0b", size=9),
            line=dict(color="#f59e0b", width=2),
            text=[_fmt_int(v) for v in df_anuais["qt_operacoes"]],
            textposition="top center",
            hovertemplate="Ano: %{x}<br>Competências: %{text}<extra></extra>",
        )
    )
    fig_line_op.update_layout(
        **_layout_plot(
            _layout_base,
            title="Evolução — Competências Omissas",
            yaxis=dict(gridcolor="rgba(148,163,184,0.08)"),
        )
    )
    st.plotly_chart(fig_line_op, use_container_width=True)
...
df_exib = df_full[["periodo", "qt_cnpj_base", "perc_cnpj_base", "qt_operacoes", "perc_operacoes"]].copy()
...
st.dataframe(
    ...
    column_config={
        ...
        "qt_operacoes": st.column_config.NumberColumn("Competências Omissas", format="%d"),
        "perc_operacoes": st.column_config.ProgressColumn(
            "% Competências", format="%.2f%%", min_value=0, max_value=100
        ),
    },
)
...
st.markdown(
    ...
    <div class="coate-panel">
        ...
        <p style="margin:0 0 0.6rem 0;">
            O total de <strong>49.119 CNPJs únicos</strong> no período 2021–2025 representa contribuintes
            distintos que foram omissos em <em>pelo menos uma competência</em> dentro da série histórica,
            <strong>sem dupla contagem</strong> entre anos.
        </p>
        <p style="margin:0 0 0.6rem 0;">
            Já a <strong>soma dos CNPJs anuais</strong> — 13.189 + 15.575 + 19.461 + 26.019 + 29.030 —
            totaliza <strong>103.274 ocorrências</strong>, pois um mesmo CNPJ que foi omisso em mais de
            um ano é contado uma vez por ano. Isso evidencia que parte significativa dos contribuintes
            reincide na omissão ao longo dos exercícios.
        </p>
        <p style="margin:0;">
            As <strong>971.866 competências omissas</strong> correspondem à soma direta de todas as
            declarações (mês/ano por CNPJ) não entregues no período, e batem exatamente com o total
            consolidado — não há deduplicação nessa dimensão.
        </p>
    </div>
    """,
    unsafe_allow_html=True,
...
# ── Footer ─────────────────────────────────────────────────────────────────────
st.markdown(
    '<div class="coate-footer">Simples Nacional · Omissão PGDAS · Painel COATE · SEFAZ-CE · Dados gerados em '
    + data_ref
    + "</div>",
    unsafe_allow_html=True,
)
